...
import { HeadingVector4 } from '../common/Icon';
        ...
        <div className='bg-faq_bg bg-no-repeat mt-[-2px] bg-cover pb-[58px] sm:pb-[88px] md:pb-[108px] lg:pb-[161px]'>
            ...
            <div className='container max-w-[1164px] mx-auto'>
                <div className='max-w-[480px] sm:max-w-[576px] md:max-w-[768px] lg:max-w-[884px] mx-auto pt-[11px] sm:pt-[41px] md:pt-[61px] lg:pt-[72px]'>
                    {FAQS.map((FAQS, index) => (
                        <div key={index} className={`overflow-hidden transition-all duration-500 ${activeIndex === index ? "border-[1px] border-sky_blue bg-accordian_gradient shadow-border" : "border-transparent border-[1px]"} mt-6`}>
                            <button onClick={() => toggleAccordion(index)} className={`${activeIndex === index ? "p-[8px_8px_0px_8px] lg:p-[21px_20px_0px_20px] border-transparent" : "p-2 lg:p-[20px_20px]"} w-full text-left bg-blur_black backdrop-blur-lg flex items-center justify-between border-[1px] border-gray transition-all duration-500`}>
                                <span className="font-orbitron font-medium text-base leading-[24px] sm:text-[18px] md:text-[20px] md:leading-[26px] lg:text-2xl lg:leading-9 text-white">{FAQS.question}</span>
                            ...
                            <div className={`overflow-hidden transition-max-height duration-500 ${activeIndex === index ? "max-h-[100px]" : "max-h-0"}`}>
